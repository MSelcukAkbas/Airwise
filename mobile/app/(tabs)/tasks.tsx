  ...
  return (
    <View style={styles.xpWrap}>
      <View style={styles.xpHeader}>
        <Text style={styles.xpLabel}>{t('tasks.xp.level', { level: level + 1 })}</Text>
        <Text style={styles.xpValue}>{t('tasks.xp.current', { current })}</Text>
      </View>
      <View style={styles.xpBg}>
        <View style={[styles.xpFill, { width: `${progress * 100}%` }]} />
      </View>
      <Text style={styles.xpNext}>{t('tasks.xp.toNext', { amount: max - (current % max) })}</Text>
    </View>
  );
}
...
export default function TasksScreen() {
      ...
      <View style={[styles.taskCard, item.status === 'completed' && styles.taskCardDone]}>
        <View style={styles.taskTop}>
          ...
          <View style={[styles.rewardChip, item.rewardType === 'freeze' && styles.rewardChipFreeze]}>
            ...
            <Text style={[styles.rewardText, item.rewardType === 'freeze' && styles.rewardTextFreeze]}>
              {item.rewardType === 'xp' ? t('tasks.card.xpReward', { amount: item.rewardAmount }) : t('tasks.card.freezeReward')}
            </Text>
          </View>
        </View>
        ...
        {isPending && (
          <View style={styles.taskActions}>
            ...
            >
              <Text style={styles.skipTaskBtnText}>{t('tasks.card.skip')}</Text>
            </TouchableOpacity>
            <TouchableOpacity
            ...
            >
              ...
              <Text style={styles.doneBtnText}>{t('tasks.card.markDone')}</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {item.status === 'completed' && (
          <View style={styles.completedBanner}>
            ...
            <Text style={styles.completedBannerText}>{t('tasks.card.claimed')}</Text>
          </View>
        )}
      </View>
    );
      ...
      <View style={styles.freezeRow}>
        <View style={styles.freezeCard}>
          ...
          <View>
            ...
            <Text style={styles.freezeLabel}>{t('tasks.stats.freeze')}</Text>
          </View>
        </View>
        <View style={styles.streakMiniCard}>
          ...
          <View>
            ...
            <Text style={styles.freezeLabel}>{t('tasks.stats.done')}</Text>
          </View>
        </View>
      </View>

      <View style={styles.sectionHeader}>
        <Text style={styles.sectionTitle}>{t('tasks.sections.daily')}</Text>
        {pendingCount > 0 && (
          <View style={styles.pendingBadge}>
            <Text style={styles.pendingBadgeText}>{t('tasks.badges.pending', { count: pendingCount })}</Text>
          </View>
        )}
      </View>
    </View>
  ...
  const ListEmpty = () => (
    <View style={styles.emptyWrap}>
      ...
      <Text style={styles.emptyText}>{t('tasks.empty.description')}</Text>
    </View>
  );

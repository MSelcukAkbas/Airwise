...
const BREATHING_PHASES = [
  { label: 'Breathe In', duration: 4000, instruction: 'Inhale slowly through your nose', scale: 1 },
  { label: 'Hold', duration: 7000, instruction: 'Hold your breath gently', scale: 1 },
  { label: 'Breathe Out', duration: 8000, instruction: 'Exhale fully through your mouth', scale: 0.55 },
];

const TRIGGER_OPTIONS: { key: TriggerReason; label: string }[] = [
  { key: 'stress', label: 'Stress' },
  { key: 'caffeine', label: 'After Coffee' },
    ...
    return (
      <Animated.View style={[styles.container, { opacity: bgOpacity }]}>
        <View style={[styles.cancelScreen, { paddingTop: topInset + 20, paddingBottom: bottomInset + 20 }]}>
          ...
          <Text style={styles.cancelTitle}>Craving Detected</Text>
          <Text style={styles.cancelSubtitle}>
            Starting 90-second barrier protocol in {cancelCountdown}...
          </Text>
          <TouchableOpacity
            style={styles.cancelBtn}
            onPress={() => router.back()}
          >
            <Text style={styles.cancelBtnText}>Cancel ({cancelCountdown})</Text>
          </TouchableOpacity>
        </View>
      </Animated.View>
    );
        ...
        <View style={[styles.barrierScreen, { paddingTop: topInset + 16, paddingBottom: bottomInset + 16 }]}>
          ...
          <View style={styles.breathingContainer}>
            ...
            <View style={styles.breathingCenter}>
              <Text style={styles.breathingLabel}>{currentPhase.label}</Text>
          ...
          <View style={styles.barrierFooter}>
            <Text style={styles.breathingInstruction}>{currentPhase.instruction}</Text>
            <Text style={styles.breathingProtocol}>4 — 7 — 8 Breathing Technique</Text>
          </View>
        </View>
      </Animated.View>
    );
    ...
    return (
      <Animated.View style={[styles.container, { opacity: bgOpacity }]}>
        <View style={[styles.triggerScreen, { paddingTop: topInset + 16, paddingBottom: bottomInset + 16 }]}>
          ...
          <Text style={styles.triggerTitle}>90 Seconds Complete</Text>
          <Text style={styles.triggerSubtitle}>
            The peak has passed. What triggered your craving?
          </Text>
          ...
          <View style={styles.triggerGrid}>
            {TRIGGER_OPTIONS.map((t) => (
              <TouchableOpacity
                ...
                onPress={() => { setSelectedTrigger(t.key); Haptics.selectionAsync(); }}
              >
                <Text style={[styles.triggerChipText, selectedTrigger === t.key && styles.triggerChipTextActive]}>
                  {t.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={styles.intensitySection}>
            <Text style={styles.intensityLabel}>Craving intensity: {intensity}/10</Text>
          ...
          <View style={styles.resultActions}>
            <TouchableOpacity style={styles.survivedBtn} onPress={handleSurvived}>
              ...
              <Text style={styles.survivedBtnText}>I Survived It</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.failedBtn} onPress={handleFailed}>
              <Text style={styles.failedBtnText}>I Still Smoked</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Animated.View>
  ...
  return (
    <Animated.View style={[styles.container, { opacity: bgOpacity }]}>
      <View style={[styles.resultScreen, { paddingTop: topInset + 20, paddingBottom: bottomInset + 20 }]}>
        ...
        {survived ? (
          <>
            <Text style={styles.resultTitle}>You made it!</Text>
            <Text style={styles.resultText}>
              You resisted the craving. This moment of discipline compounds over time.{'\n\n'}The urge has passed — you're stronger than you think.
            </Text>
          </>
        ) : (
          <>
            <Text style={styles.resultTitle}>It happens</Text>
            <Text style={styles.resultText}>
              No shame in this. What matters is that you activated the protocol — that practice alone builds tolerance over time.
            </Text>
          </>
        )}
